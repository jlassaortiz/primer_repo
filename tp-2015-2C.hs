...
pixelsDiferentesEnFrame = dis (0, 0 , )


dist :: [Pixel] -> [Pixel] -> Float -> [(Integer, Integer, PixelDelta)]
dist (p:px) (q:qx) u | norma (resta p q) > u = (resta p q) : dist px qx u
                     | norma (resta p q) <= u = dist px qx u
dist [] _ u = []
